...
public class abstractionRun {
		...
		/*
		 *  1. 객체지향언어 : "객체"를 "지향"하는 "언어"로 
		 *  						"객체 중심"으로 돌아간다고 생각하면됨.
		 *  
		 *   지양 => 좋지 않음
		 *   지향 => 좋음 (추구한다.)
		 *  
		 *  2. 객체란 : 현실세계에 독립적으로
		 *  			존재(목적이 있고 가치가 있고 의미가 있는)하는 모든것들을 의미(유형, 무형, 개념 .. )
		 ...
		 *  	: 클래스라는 틀을 먼저 만들어야됨!! (클래스란? 각 개채들의 속성(정보)들을 담아낼 그릇 같은 존재)
		 *  					=> 이름, 나이 , 주소, 핸드폰번호 등등을 담을 수 있는 그릇
		 *  	>> 어떻게 만들까? => 추상화 + 캡슐화 접목해서 만들어야함!
		 *  
		 *  5. 추상화 과정
		 *  	1) 내가 구현하고자 하는 프로그램에서 필요한 객체들을 생각해볼 것! => 쇼핑몰
		 *  	2) 그 객체들이 가지는 공통적인 속성, 기능들을 모두 추출 => 이름, 나이, 주소, 핸드폰번호, mbti..... 
		 *  	3) 추출한 것들을 가지고 내가 구현하고자 했던 프로그램 "실질적인 목적"에 맞춰 불필요한 속성, 기능들을 제거할 것 => mbti,혈액형...
		 *  	4) 최종적으로 추려진 속성들을 어떤 자료형으로, 어떤 변수명으로 사용할건지 생각 => 추상화
		 *  
		 *   ex) 학생 관리 프로그램
		 ...
		 *  	   학생 "성적"을 관리하는 프로그램이라고 한다면 : 이름, 학년, 주소, 반, 국어점수, 수학점수, 영어점수
		 ...
		 *  		이사라 라는 학생 객체를 하나 만들어내기 위해서? ==> String name3 = "이사라"; int age3 = 22; double height3 = 150.4;
		 *  		..... 등등의 수백명에 해당하는 학생들 만들어야됨!
		 *  
		 *  		어떤 문제가 발생?? => 수백명에 해당하는 학생들을 만들 때 => 수천개의 변수가 만들어짐.
		 *  
		 *  	2) "배열"로 프로그래밍을 한다면?
		 *  		* 배열 - 하나의 자료형에 여려개의 값들을 보관할 수 있음
		 * 
		 * 			학생들의 이름들을 보관하는 String[] name = {"문동은", "박연진", "이사라"};
		 * 			학생들의 나이를 보관하는 int[] age =   	 {  20,      21,      22  };
		 * 			학생들의 키를 보관하는 double[] height =  { 162.3 ,  173.5 , 150.6 };
		 * 	
		 * 			어떤 문제가 발생?
		 * 			=> 기존의 학생 정보를 지워야할 때 뒤의 정보들을 앞으로 땡기는 과정을 진행해야함...
		 * 			잘못 땡기면 정보가 틀어진다..
		 * 
		 * 			=> 새로운 학생이 전학오면? 배열을 다시 만들어야 함! + 복사까지 해야함!
		 * 
		 * 		3) 그래서 나온 개념이 구조체!!
		 * 
		 * 			* 구조체 - 여러개의 자료형 값들을 보관할 수 있음
		 * 			
	...
	public static void main(String[] args) {
		...
		// 자바에서의 객체로 만들기 위해서 new 라는 키워드를 통해 Heap 영역에 생성 먼저 해야됨!! (공간할당)
		...
		// 각 영역에 "직접 접근"해서 내가 원하는 값 대입 => public 이라서 개방되어 있음.
		// arr[0] = 10;
