...
public class EasyRun {
	...
	public static void main(String[] args) {
		...
		// 다형성 적용 시켜서 부모타입으로 자식객체를 받아주는 형태는 가능
		Sports s = new BasketBall();
		
		Sports[] arr = new Sports[2]; // 배열만 만듬
	//	 부모 			자식
		...
		/*
		 * 동적바인딩 : 컴파일 시점에서는 정적바인딩으로 현재 레퍼런스의 
		 *  						자료형의 클래스에 있는 메소드를 가리긴 함.
		 * 런타임시점 (실행시점)에서는 동적바인딩으로 가각의 자식클래스에 오버라이딩 된 메소가 있을 경우 그걸 실행함
		 * 
		 *  ● 추상클래스
		 *  - 미완성된 클래스
		 *  - 일반필드 + 일반 메소드 [+ 추상메소드]
