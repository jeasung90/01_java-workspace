...
public class BufferDao {
	 ...
	 *  			기반 스트림에서 제공하지 않는 추가적인 메소드 제공 / 데이터 전송 속도 향상 + a
	...
	public void fileSave() {
		...
		//BufferedWriter bw = new BufferedWriter("c_buffer.txt"); // 단독사용 불가 기반스트림 있어야함
		
		// 1. 기반스트림 먼저 생성
		//FileWriter fw = new FileWriter("c_buffer.txt");
		
		// 2. 보조스트림 생성시 기반 스트림 객체를 전달하면 생성
		...
		*/
		...
		// Try~with~resource 구문으로 자원 반납까지 더 간단헤게 작업할 수 있음
		// jdk7 버전 이상부터 지원!
		
		/*
		 *    [표현법]
		 *  try(try 블럭 내에서 사용할 스트림 객체 생성구문) { // 알아서 try 다 끝나면 해당 스트림 반납까지 다 해줌
		 *  
		 *  }catch(예외 클래스 e){
		 *  
		 *  }
