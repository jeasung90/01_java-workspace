...
public class InetRun {
	...
	public static void main(String[] args) {
		 ...
		 *   * 서버와 클라이언트
		 *  > 서버
		 *  	: 클라이언트(고객)에게 서비스를 제공해주는 프로그램 (클라이언트의 요청을 처리해서 응답해주는) : 서빙하는 역활..
		 *  > 클라이언트
		 *   	: 서버에 요청하는 컴퓨터 (서비스를 제공 받는 고객)
		 *
		...
		try {
			...
			// 내가 알고있는 도메인을 통해서 그 서버 관련 정보 얻기
			...
			// 도메인을 통해서 서버관련한 것들을 배열로도 받을 수 있음
			
			InetAddress[] naverHose = InetAddress.getAllByName("www.naver.com");
			
			System.out.println("네이버 호스트 개수 : "+naverHose.length);
			
			for(InetAddress a:naverHose) {
				System.out.println("네이버의 서버명 : "+a.getHostName());
				System.out.println("네이버의 IP주소 : "+a.getHostAddress());
			}
		 ...
		 *   - 웹에서의 통신방식 : HTTP 프로토콜 통신 방식
		 *   
		 *   
		 *  ● 자바만을 가지고 서오봐 클라이언트 간의 간단한 통신 해보기!!
		 *  	이 때 데이터를 입출력하고자 한다면 서버와 클라이언트 간의 스트림(연결통로)이  필요하다!!
		 *  
		 *  ★ 소켓 : 프로세스간의 통신을 담당하는 것! (스트림을 연결하기 위한 문)
		 *  
		 *  	* 소켓 프로그래밍 (TCP방식 / UDP방식)
		 *  - TCP 방식 
		 *  : 데이터의 전송속도가 느림! 데이터가 정확하고 안정적으로 전달 가능함 (신뢰성이 요구되는 프로그램 : http, 이메일)
		 *  - UDP 방식 
		 *  : 데이터의 전송속도거 빠름! 데이터가 신뢰성 없는 데이터가 전달 될 수 있음! (데이터를 빠른 속도로 전송하고자 하는 프로그램 : 실시간 동영상 서비스)
		 *  
		 *  
		 *  
		 *  
