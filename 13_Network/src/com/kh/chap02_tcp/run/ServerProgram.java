...
public class ServerProgram {
	...
	public static void main(String[] args) {
		 ...
		 * - 데이터를 교환하기에 앞서서 서버, 클라이언트가 연결되있어야함!!(서버가 먼저 실행되어 클라이언트의 요청 기다릴거임)
		 * - 신뢰성 있는 데이터를 전달 가능
		 * 
		 *  ● 소켓(Socket)
		 * - 프로세스간의 통신을 담당
		 * - Input / OutputStram 을 가지고 있음! (이 스트림을 통해 입출력이 이루어짐)
		 * 
		 * 	* Server Socket
		 * - 포트와 연결되어(Bind) 외부의 연결 요청을 기다리다 연결요청 들어오면 수락해줄 용도
		 *  	수락 => 통신 할 수있는 Client Socket 생성
		 */
		...
		try {
		// 2) Server Socket 객체 생성시 포트 결합(bind) 
		...
		// 6) 보조스트림을 통해서 성능 개선
		// 		입력용 스트림 => 1바이트 짜리 스트림을 문자스트림으로 변경해줌 => 성능 더 좋아짐
			br = new BufferedReader(new InputStreamReader(socket.getInputStream()));
		// 출력용 스트림 (클라이언테에게 값을 한 줄 단위로 출력할 수 있는 출력용 스트림)
			pw = new PrintWriter(socket.getOutputStream());
		// PrintWriter : 데이터 출력 print(), println() 메소드 가지고 있는 보조 스트림
		
		// 7) 스트림을 통해 읽고 쓰기
		// 클라이언트로부터 전달된 메세지가 있을 경우 서버측에서 읽어드릴거임! (입력)
			...
			pw.flush(); // 현재 스트림에 있는 데이터를 강제로 내보내는 메소드 => close() 자동 보내짐
			}
		} catch (IOException e) {
			e.printStackTrace();
		}finally {
