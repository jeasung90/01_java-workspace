...
public class ListRun컬렉션의정의및장점 {
	 ...
	 * - 프레임워크	: 이미 만들어져 있는 틀(뼈대) => 그냥 가져다 쓰면 됨
	 * 
	 *  * 배열의 단점과 컬렉션의 장점 
	 *  > 배열의 단점
	 *  	1. 배열은 우선 크기를 지정해야됨!! 한번 지정된 크기를 변경할 수  없음!!
	 *  		새로운 값을 더 추가하고자 한다면 새로운 크기의 배열을 만들고 기존배열을 복사하는 코드를 직접 기술해야함!
	 *   
	 *   	2. 배열 중간 위치에 값을 추가한다거나 삭제하는 경우 매번 값을 땡겨주는 복잡한 알고리즘을 직접 기술해야함...
	 *  
	 ...
	 *  		크기의 제약이 없음
	 *  
	 *  	2. 중간에 값을 추가하거나 삭제하는 경우 값을 땡겨주는 작업(알고리즘)을 직접 기술할 필요 없음!!
	...
	public static void main(String[] args) {
		ArrayList list = new ArrayList(/*크기지정 가능*/); // 크기 지정을 할수도 안할수도 있음!!! 안하면 기본크기 10으로 생성!!
		...
		System.out.println(list);  // list 특징 : 순서 유지 하면서 당김 (0번 인덱스부터 차곡 차곡)
		
		// 2. add(int index, E e ) : 직접 인덱스를 지정해서 해당 인덱스 위치에 데이터를 추가시켜주는 메소드
		list.add(1, new Music("진심이 담긴 노래", "케이시" ));
		
		// 중간 위치에 데이터 추가시 복잡한 알고리즘 직접 구현 안함 => 장점 2번
		...
		// 4. set(int index,E e) : 해당 인덱스 위치의 데이터를 새로이 전달된 e 로 수정시켜주는 메소드
		
		list.set(1, new Music("헌삥", "지야코")); 
		
		System.out.println(list); 
		
		// 5.  size() : 해당 리시트의 사이즈를 반환시켜주는 메소드(즉, 몇개의 데이터가 담겨있는지)
		System.out.println("리스트의 사이즈 : "+list.size()); // 배열과 비교해보자면 length 같은 느낌스
		...
		System.out.println((((Music) list.get(1)).getTitle())); // Object 타입이기 때문에 강제 형변환 해줘야한다.

		//비주류 7. subList(int index1, int index2) : List => 추출해서 새로운 List 반환 : substring 같은거	
		List sub = list.subList(0, 2); // 0<=    <2
		System.out.println(sub);
		
		//비주류 8. addAll(collect c) : 컬렉션을 통째로 뒤에 추가시켜주는 메소드
		list.addAll(sub);
		System.out.println(list);
		
		//★ 9. inEmpty() : boolean => 컬렉션이 비어있는지 묻는 메소드
